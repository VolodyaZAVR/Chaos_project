...
def encrypt_binary_data(binary, key, vector):
    temp_binary = [0] * len(binary)
    for i in range(len(binary)):
        temp_binary[i] = int(binary[i])

    for i in range(len(temp_binary)):
        temp_binary[i] = int(logical_xor(temp_binary[i], key[i]))
    ...
    while 128 * i + j < len(temp_binary):
        temp_binary[128 * i + j] = int(logical_xor(temp_binary[128 * i + j], temp_key[j]))
        j += 1
        if j >= 128:
            i += 1
            for j in range(128):
